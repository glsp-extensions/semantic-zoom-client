...
export class LevelOfDetail {
    ...
    private isAssignedRule(ruleSelector: string, element: SParentElement & { parent?: SParentElement }): boolean {
        ...
        if(nextSelector.includes('>')) {
            if(!element.parent || element.type !== nextSelector.substring(selectorSplitIndex+1) || element.parent.type !== nextSelector.substring(0, selectorSplitIndex)) {
                return false
            }
            if(substringIndex !== -1 && !this.isAssignedRule(ruleSelector.substring(0, substringIndex+selectorSplitIndex), element.parent)) {
                return false
            }
        }
            ...
            while(currentParent) {
                if(currentParent.type === nextSelector.substring(0, selectorSplitIndex)) {
                    if(substringIndex === -1 || this.isAssignedRule(ruleSelector.substring(0, substringIndex+selectorSplitIndex), currentParent)) {
                        parentFound = true;
                        break;
                    }
                }
